...
class ResidualBlock(nn.Module):
    def __init__(self, dim, size, bottleneck, weight_norm):
...
class ResidualModule(nn.Module):
    def __init__(self, in_dim, dim, out_dim, size,
        ...
        if res_blocks > 0:
            ...
            self.core_block = nn.ModuleList(
                [ResidualBlock(dim, size, bottleneck, weight_norm) 
...
class AbstractCoupling(nn.Module):
    ...
    def batch_stat(self, x):
        ...
        """
        mean = torch.mean(x, dim=(0, 1, 2, 3), keepdim=True)
        var = torch.mean((x - mean)**2, dim=(0, 1, 2, 3), keepdim=True)
        return mean, var
...
class CheckerboardAffineCoupling(AbstractCoupling):
    def __init__(self, device, in_out_dim, mid_dim, size, mask_config, hps):
        ...
        self.in_bn = nn.LayerNorm([in_out_dim, size, size])
        self.block = nn.Sequential(        # 1st half of resnet: shift
            nn.ReLU(),                    # 2nd half of resnet: log_rescale
            ResidualModule(2*in_out_dim+1, mid_dim, 2*in_out_dim, size,
                self.res_blocks, self.bottleneck, self.skip, self.weight_norm))
        self.out_bn = nn.BatchNorm2d(in_out_dim, affine=False)
    ...
    def forward(self, x, reverse=False):
        ...
        if reverse:
            if self.coupling_bn:
                running_mean = self.out_bn.running_mean
                running_var = self.out_bn.running_var
                mean = torch.mean(running_mean).reshape(1, 1, 1, 1)
                var = (running_var + running_mean ** 2).mean() - torch.mean(running_mean) ** 2
            ...
            if self.coupling_bn:
                if self.training:
                    ...
                    var = (running_var + running_mean ** 2).mean() - torch.mean(running_mean) ** 2
                    var = var.reshape(1, 1, 1, 1)
                x = (x - mean) / torch.sqrt(var + 1e-5) * (1. - mask) + x * mask
                log_diag_J = log_diag_J - 0.5 * torch.log(var + 1e-5) * (1. - mask)
        return x, log_diag_J
...
class ChannelwiseAffineCoupling(AbstractCoupling):
    def __init__(self, device, in_out_dim, mid_dim, size, mask_config, hps):
        ...
        self.scale = nn.Parameter(torch.zeros(1), requires_grad=True)
        self.scale_shift = nn.Parameter(torch.zeros(1), requires_grad=True)
        self.in_bn = nn.LayerNorm([in_out_dim//2, size, size])
        self.block = nn.Sequential(        # 1st half of resnet: shift
            nn.ReLU(),                    # 2nd half of resnet: log_rescale
            ResidualModule(in_out_dim, mid_dim, in_out_dim, size,
                self.res_blocks, self.bottleneck, self.skip, self.weight_norm))
        self.out_bn = nn.BatchNorm2d(in_out_dim//2, affine=False)
...
class RealNVP_LN(nn.Module):
    def __init__(self, device, datainfo, prior, hps):
        ...
        Args:
            ...
            hps: the set of hyperparameters.
        """
        super(RealNVP_LN, self).__init__()
        self.device = device
        ...
        if datainfo.name == 'cifar10' or datainfo.name == "mnist":
            ...
            size //= 2
            self.s1_chan = self.channelwise_combo(chan*4, dim, size, hps)
    ...
    def channelwise_combo(self, in_out_dim, mid_dim, size, hps):
        ...
        """
        return nn.ModuleList([
                ChannelwiseCoupling(self.device, in_out_dim, mid_dim, size, 0., hps),
                ChannelwiseCoupling(self.device, in_out_dim, mid_dim, size, 1., hps),
                ChannelwiseCoupling(self.device, in_out_dim, mid_dim, size, 0., hps)])
